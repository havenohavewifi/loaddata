...
int initRelation(struct dbSysHead *head, long fid, char *relationName, char *constructorName);//初始化关系
int changeRecordNum(struct dbSysHead *head, long fid, int num);//更改记录总数
int initAttribute(struct dbSysHead *head, long fid, char *name, int type, int length);//初始化一个属性表
